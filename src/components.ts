...
import { getCurrentEndpoint, request } from "./utilities";
...
export function createEndpointsList(cfg: URLConfigType): HTMLElement {
    ...
    ul.style.cssText = "list-style:none;padding:0;margin:0;max-height:240px;overflow:auto";
  
    cfg.urls.forEach((u) => {
      const li = document.createElement("li");
      li.textContent = u.label;
      li.title = u.value;
      li.style.cssText = "padding:8px 10px;border-radius:8px;margin:4px;cursor:pointer";
...
export function toast(msg: string) {
    ...
    Object.assign(t.style, {
      ...
      bottom: "16px", background: "#111", color: "#f3f3f3",
      padding: "10px 14px", borderRadius: "10px", zIndex: "2147483647",
      opacity: "0", transition: "opacity .25s ease, transform .25s ease"
    } as CSSStyleDeclaration);
    document.body.appendChild(t);
    requestAnimationFrame(() => { t.style.opacity = "1"; (t.style as any).transform = "translateX(-50%) translateY(0)"; });
    setTimeout(() => { t.style.opacity = "0"; setTimeout(() => t.remove(), 250); }, 1400);
  }

  export function createRoot(): HTMLDivElement {
    const root = document.createElement("div");
    root.style.cssText = "position:fixed;right:16px;bottom:54px;z-index:2147483647";
    return root;
  }